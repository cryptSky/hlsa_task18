...
priv_stmt='GRANT REPLICATION SLAVE ON *.* TO "mydb_slave_user1"@"%" IDENTIFIED BY "mydb_slave_pwd"; FLUSH PRIVILEGES;GRANT REPLICATION SLAVE ON *.* TO "mydb_slave_user2"@"%" IDENTIFIED BY "mydb_slave_pwd"; FLUSH PRIVILEGES;'
...
start_slave1_stmt="CHANGE MASTER TO MASTER_HOST='$(docker-ip mysql_master)',MASTER_USER='mydb_slave_user1',MASTER_PASSWORD='mydb_slave_pwd',MASTER_LOG_FILE='$CURRENT_LOG',MASTER_LOG_POS=$CURRENT_POS; START SLAVE;"
...
start_slave2_stmt="CHANGE MASTER TO MASTER_HOST='$(docker-ip mysql_master)',MASTER_USER='mydb_slave_user2',MASTER_PASSWORD='mydb_slave_pwd',MASTER_LOG_FILE='$CURRENT_LOG',MASTER_LOG_POS=$CURRENT_POS; START SLAVE;"
